...
# Detail sheet uses the production shader on the actual mature mesh.
y=np.deg2rad(78)
side_rotation=np.array([[np.cos(y),0,np.sin(y)],[0,1,0],[-np.sin(y),0,np.cos(y)]])
profile=render((points-[0,.1,.65])@(rx@side_rotation).T,indices,colors,uv,[-1.65,-1.65,3.3,3.3],(720,720))
board=Image.new('RGB',(1440,840),(11,13,12));board.paste(single,(0,65));board.paste(profile,(720,65))
draw=ImageDraw.Draw(board)
try:font=ImageFont.truetype('/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf',20)
except OSError:font=ImageFont.load_default()
draw.text((30,25),'NARCISSUS / SOLID PETALS + DETAILED STAMENS',font=font,fill=(225,225,212))
draw.text((765,25),'SIDE VIEW / CLOSED LEAF AND PETAL EDGES',font=font,fill=(225,225,212))
draw.text((30,800),'ACTUAL PROCEDURAL MESH / MESA SHADER PREVIEW / NOT UNITY CAMERA CAPTURE',font=font,fill=(140,148,138))
output.parent.mkdir(parents=True,exist_ok=True);board.save(output)
print('PASS: actual exported detailed mesh + production fragment shader; preview',output)
